...
{
    //var marker_color = $('.marker-selected').children(":first").children(":first").attr('alt');
});
!function (t, e) {
    L.drawVersion = "0.2.4-dev", L.drawLocal = {
        ...
        addHooks: function () {
			//Click on toolbar for marker
			//L.AwesomeMarkers.icon({icon: 'coffee', markerColor: 'red', prefix: 'fa', spin:true})
			...
			L.Draw.Feature.prototype.addHooks.call(this), this._map && (this._tooltip.updateContent({text: L.drawLocal.draw.handlers.marker.tooltip.start}), this._mouseMarker || (this._mouseMarker = L.marker(this._map.getCenter(), {
                icon: L.AwesomeMarkers.icon({icon: 'coffee', markerColor: 'red', prefix: 'fa', spin:true}), opacity: 0, zIndexOffset: this.options.zIndexOffset
            })), this._mouseMarker.on("click", this._onClick, this).addTo(this._map), this._map.on("mousemove", this._onMouseMove, this))
        },
        removeHooks: function () {
        ...
        _onMouseMove: function (t) {
            ...
            //override with choice of awesome marker
           
            if ($("#custom-icon-input").val().length < 1)
            {
                this.options.icon = L.AwesomeMarkers.icon({
                                                            icon: $('.icon-selected').children(":first").children(":first").attr('class').split(' ')[1].split('-')[1], 
                                                            markerColor: $('.marker-selected').children(":first").children(":first").attr('alt'), 
                                                            prefix: 'fa'
                });
            }else{
        ...
        getActions: function (t) {
            //The call to bring up the side bar
